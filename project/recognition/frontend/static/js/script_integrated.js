...
const API_CONFIG = {
    ...
    ENDPOINTS: {
        ...
        GAZE_ACCURACY: '/api/gaze/accuracy'
    }
};
...
// 检查URL参数，如果是全屏模式则自动全屏
if (window.location.hash === '#fullscreen') {
    document.addEventListener('DOMContentLoaded', function() {
        setTimeout(enterFullscreen, 1000); // 延迟1秒全屏
    });
}
...
async function initializeSystem() {
    try {
        ...
        // 检查后端连接 - 使用系统状态API
        const response = await fetch(`${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.SYSTEM_STATUS}`);
        if (response.ok) {
...
async function checkGazeAccuracy(targetX, targetY) {
    ...
    try {
        const response = await fetch(`${API_CONFIG.BASE_URL}${API_CONFIG.ENDPOINTS.GAZE_ACCURACY}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
